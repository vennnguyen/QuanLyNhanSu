...
public class SUPPORT {
	...
	public static SalaryDTO chuyenBangChamCongSangLuong(TimeSheet bcc, double thuong, double phucapkhac, double khoantrukhac) {
		SalaryDTO x = new SalaryDTO();
		x.setSalaryId("L" + bcc.getTimesheetId().substring(3));
		x.setTimeSheetId(bcc.getTimesheetId());
		
		double dulieu[] = EmployeeDAO.getLuongNhanVien(bcc.getEmployeeId());
		double luongThucTe = Math.round(dulieu[0] / 26 * bcc.getWorkingDays() + (dulieu[0] / 26 / 8 * bcc.getOvertimeHours()));
		...
		x.setActualSalary(luongThucTe);
		x.setPositionAllowance(phuCap);
		x.setOtherAllowances(phucapkhac);
		x.setInsuranceDeduction(truBaoHiem);
		x.setOtherDeductions(khoantrukhac);
		x.setBonus(thuong);
		x.setTax(thue);
		x.setNetSalary(thucLanh);
		return x;
	}
}